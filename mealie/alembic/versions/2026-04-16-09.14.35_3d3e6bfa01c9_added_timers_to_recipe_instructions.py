...
from mealie.db.models.recipe.instruction_timer import RecipeInstructionTimer
...
class RecipeInstruction(SqlAlchemyBase):
    ...
    timers: orm.Mapped[list["RecipeInstructionTimer"]] = orm.relationship("RecipeInstructionTimer", single_parent=True)
...
def parse_instructions(session: orm.Session):
    ...
    instruction_timers_data: list[dict] = []
    ...
    for instruction in session.query(RecipeInstruction).all():
        ...
        for timer in timers:
            instruction_timers_data.append({"id": uuid4(), "duration": timer, "recipe_instruction_id": instruction.id})

    op.bulk_insert(RecipeInstructionTimer.__table__, instruction_timers_data)
    session.commit()
...
def upgrade():
    ...
    with op.batch_alter_table("recipe_instruction_timers", schema=None) as batch_op:
        batch_op.create_index(batch_op.f("ix_recipe_instruction_timers_created_at"), ["created_at"], unique=False)
        batch_op.create_index(
            batch_op.f("ix_recipe_instruction_timers_recipe_instruction_id"), ["recipe_instruction_id"], unique=False
        )
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table("recipe_instruction_timers", schema=None) as batch_op:
        batch_op.drop_index(batch_op.f("ix_recipe_instruction_timers_recipe_instruction_id"))
        batch_op.drop_index(batch_op.f("ix_recipe_instruction_timers_created_at"))

    op.drop_table("recipe_instruction_timers")
    # ### end Alembic commands ###
